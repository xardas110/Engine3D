...
#define CHECKJSON(json, key, type) \
if (!json.Contains(key)) \
	printf("json key not found %s\n", key);\
if (json[key].GetType() != type)\
	printf("json type doesn't match %i != %i, key: %s \n", json.GetType(), type, key)
...
public:
	//uint removed, only caused bugs
	enum Type { Null, Bool, Int, Uint, Float, String, Array, Object };
