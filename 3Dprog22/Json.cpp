...
Json Json::Load(const std::string & path)
{
	...
	{
		std::cout << "Failed to open JSON at path: " << path << std::endl;
	}
...
void Json::Print() const
{
	...
	case Type::Int:
		...
		break;
	case Type::Uint:
		Text += std::to_string(var_->uIntVal);
		break;
	case Type::Float:
...
void Json::PrintS(std::string & text, size_t & offset, const Json & json)
{
	...
	case Type::Int:
		...
		break;
	case Type::Uint:
		text += std::to_string(json.var_->uIntVal);
		break;
	case Type::Float:
...
const std::string Json::Stringify()
{
	...
	case Type::Int:
		...
		break;
	case Type::Uint:
		for (const auto& ch : std::to_string(var_->uIntVal))
		{
			result.push_back(ch);
		}
		break;
...
bool Json::operator==(const Json & other) const
{
	...
	case Type::Uint:	return var_->uIntVal == other.var_->uIntVal;
